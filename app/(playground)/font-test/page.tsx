      ...
      <div className="max-w-2xl mx-auto">
        ...
        <header className="mb-16">
          ...
          <p className="font-mono text-sm tracking-tight opacity-60 leading-relaxed max-w-lg">
            same prose, eleven fonts. scroll through and pick the one that feels like home. the first one is your current
            font for reference. Use the vertical dash on the right to jump to a font by number.
          </p>
        </header>

        <JumpRail items={fonts.map(({ name }, i) => ({ id: `font-${i + 1}`, label: `${i + 1}. ${name}` }))} ariaLabel="Jump to font" />
            ...
            <section key={name} id={`font-${i + 1}`}>
              ...
              <div className="mb-6 pb-4 border-b" style={{ borderColor: "var(--stone-200)" }}>
                <div className="flex items-baseline gap-3 flex-wrap">
                  <span className="font-mono text-xs tracking-wide uppercase opacity-40">{String(i + 1).padStart(2, "0")}</span>
                  <h2 className="font-mono text-lg font-bold tracking-tighter">{name.toLowerCase()}</h2>
                  <span
              ...
              {/* Title sample */}
              <h3 className={`${font.className} text-2xl font-bold mb-6`} style={{ color: "var(--prose-heading)", lineHeight: 1.3 }}>
