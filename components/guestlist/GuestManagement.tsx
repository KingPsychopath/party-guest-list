...
import { useGuestManagement } from '@/hooks/guestlist/useGuestManagement';
  ...
  return (
    <form onSubmit={handleAdd} className="space-y-4">
      <div>
        ...
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="Enter guest name" className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" />
      </div>
      <div>
        <label className="block text-sm font-medium text-stone-700 mb-1.5">Full Name (optional)</label>
        <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Full legal name if different" className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" />
      </div>
      <TypeaheadInput value={plusOneOf} onChange={setPlusOneOf} suggestions={mainGuestNames} placeholder="Leave empty for main guest" label="Guest of (for +1s)" />
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <input type="text" value={removeSearch} onChange={(e) => { setRemoveSearch(e.target.value); setRemoveId(''); }} placeholder="Type to search..." className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all" />
      </div>
      {filteredForRemoval.length > 0 && (
        <div className="border border-stone-200 rounded-xl max-h-48 overflow-y-auto">
          {filteredForRemoval.map((guest) => (
            <button key={guest.id} onClick={() => setRemoveId(guest.id)} className={`w-full text-left px-4 py-3 border-b border-stone-100 last:border-b-0 transition-colors ${removeId === guest.id ? 'bg-red-50 text-red-700' : 'hover:bg-stone-50'}`}>
              {guest.displayName}
            </button>
          ))}
        </div>
      )}
      <button onClick={handleRemove} disabled={!removeId} className="w-full bg-red-600 text-white py-3 rounded-xl font-medium hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="space-y-4">
      <div className="bg-stone-50 border-2 border-dashed border-stone-200 rounded-2xl p-8 text-center">
        ...
        <input ref={fileInputRef} type="file" accept=".csv" onChange={handleFileUpload} disabled={uploading} className="hidden" id="csv-upload" />
        <label htmlFor="csv-upload" className={`inline-flex items-center gap-2 bg-amber-600 text-white px-6 py-3 rounded-xl font-medium cursor-pointer hover:bg-amber-700 transition-colors ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
          {uploading ? (
...
function DataTab({
  ...
  return (
    <div className="space-y-4">
      <button onClick={handlePartyReset} disabled={dataLoading} className="w-full py-4 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-zinc-950 font-bold text-lg rounded-2xl shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 disabled:opacity-50 transition-all flex items-center justify-center gap-3">
        {dataLoading ? <div className="w-6 h-6 border-2 border-zinc-950/30 border-t-zinc-950 rounded-full animate-spin" /> : <><span className="text-2xl">*</span>Party Reset</>}
      </button>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 space-y-3">
        ...
        <button onClick={handleBootstrap} disabled={dataLoading} className="w-full bg-emerald-600 text-white py-2.5 rounded-xl font-medium hover:bg-emerald-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
          {dataLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>}
          Bootstrap from CSV
        </button>
      </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl p-4 space-y-3">
        ...
        <button onClick={handleForceReload} disabled={dataLoading} className="w-full bg-red-600 text-white py-2.5 rounded-xl font-medium hover:bg-red-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
          {dataLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>}
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-r from-pink-50 to-purple-50 border border-purple-200 rounded-xl p-4">
        ...
        <button onClick={fetchBestDressedData} className="w-full py-2 text-sm text-purple-600 hover:text-purple-800 transition-colors">Refresh</button>
      </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl p-4 space-y-3">
        ...
        <button onClick={handleWipeBestDressed} disabled={gamesLoading} className="w-full bg-red-600 text-white py-2.5 rounded-xl font-medium hover:bg-red-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex gap-1 bg-stone-100 p-1 rounded-xl">
                {TABS.map((tab) => (
                  ...
                  >
                    {tab === 'add' ? 'Add' : tab === 'remove' ? 'Remove' : tab === 'import' ? 'Import' : tab === 'data' ? 'Data' : 'Games'}
                  </button>
                ))}
              </div>
